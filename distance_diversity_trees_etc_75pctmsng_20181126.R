...
# vcf.75pctmsng <- read.vcfR("vcftools_allsocal_303indv_highqual_20pctmsng_SNPsinallbignativepops_oneSNP_nosingletons_20181009.recode.vcf")
# genind.75pctmsng <- vcfR2genind(vcf.75pctmsng) # 1672 SNPs
# indNames(genind.75pctmsng) <- popcoords
# pop(genind.75pctmsng) <- popcoords
...
privatealleles.75pctmsng <- poppr::private_alleles(genind.75pctmsng.allsocal,
                                                   alleles ~ .,
                                                   report="table",
                                                   level="population",
                                                   count.alleles=TRUE,
                                                   drop=TRUE)
privatealleles.counts.75pctmsng <- rowSums(privatealleles.75pctmsng)

#### Genetic diversity
# Might just use DnaSP 6 - nevermind, I don't trust it... numbers don't make sense

hierfstat.75pctmsng <- genind2hierfstat(genind.75pctmsng.allsocal)
...
write.csv(arhohsfissamplesize.pops.75pctmsng, file="arhohsfis_20pctmsng_SNPsinallbignativepops.csv")
...
bigpoplist <- as.character(popcounts$PopLevel3geogenclusters[popcounts$n >= 3])


genind.75pctmsng.bigpops <- popsub(genind.75pctmsng.allsocal, sublist=bigpoplist, drop=TRUE)
genpop.75pctmsng.bigpops <- genind2genpop(genind.75pctmsng.bigpops)

gl.75pctmsng.bigpops <- popsub(gl.75pctmsng.allsocal, sublist=bigpoplist, drop=TRUE)
ploidy(gl.75pctmsng) <- 2
pop(gl.75pctmsng) <- popcoords$PopLevel3geogenclusters
gl.75pctmsng@other$xy <- popcoords[,c("LongitudePopLevel3", "LatitudePopLevel3")]

hierfstat.75pctmsng.bigpops <- genind2hierfstat(genind.75pctmsng.bigpops)
...
genind.IM0103 <- genind.75pctmsng.seppops$IM0103
pop(genind.IM0103) <- indNames(genind.IM0103)
hierfstat.IM0103 <- genind2hierfstat(genind.IM0103)
mean(genet.dist(hierfstat.IM0103, method="Dch"))
hist(genet.dist(hierfstat.IM0103, method="Da"), breaks=100)

genind.MORO <- genind.75pctmsng.seppops$MORO
pop(genind.MORO) <- indNames(genind.MORO)
hierfstat.MORO <- genind2hierfstat(genind.MORO)
par(mfrow=c(2,2))
distance.reynold.MORO <- genet.dist(hierfstat.MORO, method="Fst") # equivalent (mostly) to Reynold's distance
mean(distance.reynold.MORO) 
hist(distance.reynold.MORO, breaks=100)
distance.Da.MORO <- genet.dist(hierfstat.MORO, method="Da") # equivalent (mostly) to Reynold's distance
mean(distance.Da.MORO) 
hist(distance.Da.MORO, breaks=100)
distance.prevosti.MORO <- genet.dist(hierfstat.MORO, method="Cp") # equivalent (mostly) to Reynold's distance
mean(distance.prevosti.MORO) 
hist(distance.prevosti.MORO, breaks=100)
distance.x2.MORO <- genet.dist(hierfstat.MORO, method="X2") # equivalent (mostly) to Reynold's distance
mean(distance.x2.MORO) 
hist(distance.x2.MORO, breaks=100)
distance.neifst.MORO <- genet.dist(hierfstat.MORO, method="Nei87") # equivalent (mostly) to Reynold's distance
mean(distance.wcfst.MORO) 
hist(distance.wcfst.MORO, breaks=100)
